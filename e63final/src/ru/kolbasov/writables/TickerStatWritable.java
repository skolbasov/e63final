	...
	public TickerStatWritable(Iterable<CorrelationWritable> values) {
		...
		for (CorrelationWritable value:values){
			if (value.getTicker().contains("USDRUB_TOD")){
				usdClosePrices.add(new priceTime(value.getPrice().getClosePrice(),value.getTime()));
			}
			if (value.getTicker().contains("Индекс ММВБ")){
				micexClosePrices.add(new priceTime(value.getPrice().getClosePrice(),value.getTime()));
			}
			if (value.getTicker().contains("EURRUB_TOD")){
				eurClosePrices.add(new priceTime(value.getPrice().getClosePrice(),value.getTime()));
			}
		}
	}
	@Override
